...
public class TokenServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		try {
			...
			if (validateClient(request, code, clientId, userId)) {
				...
				oauth2Token
						.setExpiredIn(new Date().getTime() + 60 * 60 * 1000L);
				...
				String error = "error";
				r = OAuthResponse.errorResponse(400).setError(error)
						.buildJSONMessage();
				response.setStatus(r.getResponseStatus());
	...
	private boolean validateClient(HttpServletRequest request, String code,
		...
		try {
			...
			if (map != null) {
				...
				if (oauth2Info != null) {
					...
					Gson gson = new Gson();
					Oauth2Info info = gson.fromJson(oauth2Info.toString(),
							Oauth2Info.class);
					System.out.println(info.getClient_id()
							+ "++++++++++++++++++++++++" + info.getUserId());
					if (userId == Long.valueOf(info.getUserId())
							&& Validator.isNotNull(info.getClient_id())
							&& info.getClient_id().equals(clientId)) {

						if (Oauth2ClientLocalServiceUtil
								.getOauth2Client(clientId) != null) {
							flag = true;
